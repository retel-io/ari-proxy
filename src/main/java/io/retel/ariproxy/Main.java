  ...
  private static void runAriEventProcessor(
    ...
    final RunnableGraph<NotUsed> processingPipeline =
        WebsocketMessageToProducerRecordTranslator.eventProcessing(
            system, applicationReplacedHandler, callContextProvider, metricsService, source, sink);

    Match(Try.of(() -> processingPipeline.run(system)))
        .of(
            Case(
                $Success($()),
                mat -> run(() -> system.log().debug("Successfully started ari event processor."))),
            Case(
                $Failure($(instanceOf(KafkaException.class))),
                err ->
                    run(
                        () -> {
