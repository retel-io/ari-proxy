...
// see:
// https://wiki.asterisk.org/wiki/display/AST/Asterisk+15+REST+Data+Models#Asterisk15RESTDataModels-Message
class AriMessageTypeTest {
  ...
  private static final String BODY_WITH_BRIDGE_ID =
      String.format("{ \"bridge\":  { \"id\": \"%s\" }  }", BRIDGE_ID);
  private static final String BODY_WITH_CHANNEL_ID =
      String.format("{ \"channel\": { \"id\": \"%s\" } }", CHANNEL_ID);
  private static final String BODY_WITH_PEER_ID =
      String.format("{ \"peer\": { \"id\": \"%s\" } }", PEER_ID);
  private static final String BODY_WITH_PLAYBACK_ID =
      String.format("{ \"playback\": { \"id\": \"%s\" } }", PLAYBACK_ID);
  ...
  void ensureExtractResourceIdFromBodyWorksForAnyType(
      String type, String body, String expectedResourceId) throws IOException {
    assertThat(
        AriMessageType.fromType(type).extractResourceIdFromBody(reader.readTree(body)),
  ...
  void ensureUnknownMessageResultsInRuntimeException() {
    MatcherAssert.assertThat(
        AriMessageType.UNKNOWN.extractResourceIdFromBody(mock(JsonNode.class)).get().getCause(),
