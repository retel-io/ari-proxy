...
public class AriCommandResponseKafkaProcessor {
	...
	public static final String EVENTS_AND_RESPONSES_TOPIC =
		"events-and-responses-topic";
	...
	private static final ObjectReader reader = mapper.readerFor(
		AriCommandEnvelope.class
	);
	private static final ObjectWriter ariMessageEnvelopeWriter = mapper.writerFor(
		AriMessageEnvelope.class
	);
	...
	private static ActorMaterializer run(
		...
		final String eventsAndResponsesTopic = kafkaConfig.getString(
			EVENTS_AND_RESPONSES_TOPIC
		);
		...
		source
			...
			.map(
				msgEnvelope -> {
					AriCommandResponseProcessing
						...
						).onFailure(error -> { throw new IllegalStateException(error); });
			...
			.mapAsync(
				...
				rawHttpResponseAndContext ->
					toAriResponse(rawHttpResponseAndContext, materializer)
			)
			.map(
	...
	private static ProducerRecord<String, String> envelopeAriResponseToProducerRecord(
		...
		final CallContextAndCommandRequestContext context =
			ariResponseAndContext._2;

		AriMessageEnvelope ariMessageEnvelope = envelopeAriResponse(
			ariResponse,
			context,
			commandsTopic
		);
	...
	private static AriCommandEnvelope unmarshallAriCommandEnvelope(
		...
		return Try
			.of(() -> (AriCommandEnvelope) reader.readValue(record.value()))
	...
	private static AriMessageEnvelope envelopeAriResponse(
		...
		final Option<AriCommandResource> maybeResource = context
				.getAriCommand()
				.extractResource();
	...
	private static String marshallAriMessageEnvelope(
		AriMessageEnvelope messageEnvelope
	) {
		return Try
			.of(() -> ariMessageEnvelopeWriter.writeValueAsString(messageEnvelope))
			.getOrElseThrow(
				t -> new RuntimeException("Failed to serialize AriResponse", t)
			);
	}
							...
							rawBody ->
								Try
									...
									.getOrElseGet(
										t ->
											Future
												.<Tuple2<AriResponse, CallContextAndCommandRequestContext>>failed(
													t
												)
												.toCompletableFuture()
									)
	...
	private static HttpRequest toHttpRequest(
		...
		return HttpMethods
			...
			.map(
				validHttpMethod ->
					HttpRequest
						...
						.addCredentials(
							HttpCredentials.createBasicHttpCredentials(user, password)
						)
			...
			.orElseThrow(
				() ->
					new RuntimeException(String.format("Invalid http method: %s", method))
			);
	}
}
